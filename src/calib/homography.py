...
def _cv2_putText(img, text, org, fontFace, fontScale, color, thickness=1, lineType=None):
    ...
    if CV2_AVAILABLE and cv2 is not None and hasattr(cv2, "putText"):
        cv2.putText(
            ...
            color,
            thickness,
            lineType or getattr(cv2, "LINE_AA", 8),
        )
...
class HomographyCalibrator:
    ...
    def load_calibration(
        cls, path: Path, config: dict[str, Any] | None = None
    ...
    def visualize_calibration(
        self, image: np.ndarray, output_path: Path | None = None
    ) -> np.ndarray:
        """
        ...
        for i, pt in enumerate(self.image_points):
            ...
            _cv2_putText(
                ...
                f"I{i + 1}",
                tuple(map(int, pt)),
                _cv2_font_hershey_simplex(),
                0.5,
                (0, 255, 0),
                1,
                None,
            )  # Draw projected world points in red
        ...
        _cv2_putText(
            vis, info_text, (10, 30), _cv2_font_hershey_simplex(), 0.6, (255, 255, 255), 2, None
        )
...
def compute_homography(
    ...
    matrix, mask = _cv2_find_homography(src_points, dst_points, ransac_threshold)
