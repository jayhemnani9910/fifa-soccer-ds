...
class AudioExtractor:
    ...
    def transcribe_audio(self, audio_path: Path, language: str | None = None) -> dict[str, Any]:
        ...
        try:
            ...
            if transcription["segments"]:
                transcription["duration"] = max(seg["end"] for seg in transcription["segments"])
            else:
    ...
    def classify_soccer_audio(
        self, transcription: dict[str, Any], audio_features: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        LOGGER.info(
            "Audio classification: %.2f confidence (%s)",
            confidence,
            "soccer" if classification["is_soccer"] else "non-soccer",
        )

        return classification

    def process_sample_audio(
        self, video_path: Path, duration_seconds: float = 60.0
    ) -> dict[str, Any]:
        ...
        try:
            LOGGER.info(
                "Processing audio sample from: %s (%.1f seconds)", video_path, duration_seconds
            )
...
def classify_youtube_audio(
    transcription: dict[str, Any], audio_features: dict[str, Any]
) -> dict[str, Any]:
    """Classify if audio content is soccer-related.
