...
class YouTubeMetadataParser:
    ...
    def _analyze_engagement(self, video_info: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "engagement_score": self._calculate_engagement_score(
                like_rate, comment_rate, views_per_minute
            ),
        }

    def _calculate_engagement_score(
        self, like_rate: float, comment_rate: float, views_per_minute: float
    ) -> float:
    ...
    def predict_soccer_content(self, metadata: dict[str, Any]) -> dict[str, Any]:
        ...
        prediction_score = (
            ...
            + (1.0 if content_type in ["highlights", "full_match"] else 0.0)
            * 0.2  # Content type (20%)
        )
        ...
        prediction = {
            ...
            "reasoning": self._generate_prediction_reasoning(
                keyword_score, channel_score, content_type
            ),
        }

        LOGGER.info(
            "Soccer prediction: %.2f confidence (%s)",
            prediction["confidence"],
            "soccer" if is_soccer else "non-soccer",
        )

        return prediction

    def _generate_prediction_reasoning(
        self, keyword_score: float, channel_score: float, content_type: str
    ) -> list[str]:
