...
class TestGraphMemory:
    ...
    def test_small_graph_memory(self):
        ...
        assert (
            result1["edge_index"].shape[1] <= result2["edge_index"].shape[1] * 1.2
    ...
    def test_large_graph_memory_limit(self):
        ...
        result = build_track_graph_optimized(
            track_windows, window=30, distance_threshold=80.0, max_nodes_per_frame=50
        )
    ...
    def test_temporal_edge_consistency(self):
        ...
        result = build_track_graph_optimized(track_windows, include_temporal_edges=True)

        # Should have some temporal edges
        assert result["edge_index"].shape[1] > 0

        # Disable temporal edges
        result_no_temp = build_track_graph_optimized(track_windows, include_temporal_edges=False)

        # Should have fewer edges
        assert result_no_temp["edge_index"].shape[1] <= result["edge_index"].shape[1]

    def _create_mock_tracks(
        self, num_frames: int, tracks_per_frame: int, with_temporal: bool = False
    ):
...
class TestGraphMemoryIntegration:
    ...
    def test_memory_comparison(self):
        ...
        result_orig = build_track_graph(track_windows, window=30, distance_threshold=80.0)

        # Optimized function
        result_opt = build_track_graph_optimized(track_windows, window=30, distance_threshold=80.0)
